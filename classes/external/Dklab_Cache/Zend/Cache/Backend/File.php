 ...
 */
require_once DKCACHE_PATH.'Zend/Cache/Backend/Interface.php';

/**
 * @see Zend_Cache_Backend
 */
require_once DKCACHE_PATH.'Zend/Cache/Backend.php';
...
class Zend_Cache_Backend_File extends Zend_Cache_Backend implements Zend_Cache_Backend_Interface
{
    ...
    public function save($data, $id, $tags = array(), $specificLifetime = false)
    {
        ...
        /**
         * This code supply fast tags deleting
         * Now, writing in separate folder is happening
         * @author Tim
         */
        if($tags)
        {
            ...
            foreach($tagList as $value){
                $file_name = CACHE_DIR.CACHE_TAG_DIR.md5($value).'.tag';
    ...
    private function _clean($dir, $mode = Zend_Cache::CLEANING_MODE_ALL, $tags = array())
    {
        ...
        /**
         * This code supply fast tags deleting
         * Now, whole deleting or deleting by tags is happening
         * @author Tim
         */
        if($mode == Zend_Cache::CLEANING_MODE_ALL)
        {
            ...
            while($item = readdir($dh))
            {
                if(($item == '.' or $item == '..'))
                        continue;
                if(is_dir($dir.$item)){
                    $result = $result and $this->_clean($dir.$item.'/');
                    @rmdir($dir.$item.'/');
                }
                else
                {
           ...
           closedir($dh);
           @mkdir(CACHE_DIR.CACHE_TAG_DIR);
           return $result; 
        }
        else if($mode ==  Zend_Cache::CLEANING_MODE_MATCHING_TAG and $tags)
        {
            $result = true;
            $tagList = array();
            if(is_array($tags))
            ...
            foreach($tagList as $value){
                ...
                {
                    @$deletableData = explode("\n", fread(fopen($file_name, "rb"), filesize($file_name)));
                    
                    if($deletableData)
                        foreach($deletableData as $value)
                        {
                            $result = $result and $this->remove($value);
                        } 
                    @unlink($file_name);    
                }
            }
